// Mudamos para Assistência!
const CONSELHO_ATUAL = "Assistência"; 

// ============================================================================
// ESCALA FIXA DA ASSISTÊNCIA E MOTOR DE DATAS
// ============================================================================
const ESCALA_ASSISTENCIA = [
...
async function carregarDadosPagina() {
    ...
    const periodoAtual = getPeriodoDaSemana(); // Pegamos a data limite da semana logo no início!
    ...
    db.collection('conselho').doc('painel_registros').collection('historico')
      ...
      .onSnapshot((snapshot) => {
        ...
        let itensMostrados = 0; // Contador para saber se a semana tá vazia
        ...
        if (!snapshot.empty) {
            snapshot.forEach(doc => {
                ...
                // ==========================================
                // O FILTRO MÁGICO: Se a data do post for ANTES do domingo atual 
                // ou DEPOIS do sábado atual, a gente ignora e não mostra na tabela!
                // ==========================================
                if (!dataObj || dataObj < periodoAtual.inicio || dataObj > periodoAtual.fim) {
                    return; // Pula para a próxima postagem (esconde as velhas)
                }

                itensMostrados++; // Achou um registro dessa semana!
        ...
        // Se depois de filtrar não sobrou nada DAQUELA semana
        if (itensMostrados === 0) {
    ...
    // =========================================================================
    // 2. BUSCAR OS RESPONSÁVEIS (LÊ DA ESCALA ATIVA NO QUADRO PRINCIPAL)
    // =========================================================================
    const idConselho = "conselho_" + CONSELHO_ATUAL.replace(/\s+/g, '');
    const idEstagiario = "estagiario_" + CONSELHO_ATUAL.replace(/\s+/g, '');

    // Lê fixo do painel de escalas principal para não apagar quando a semana virar!
...
// ============================================================================
// FUNÇÕES DE LIDERANÇA: APAGAR E EDIÇÃO INLINE
// ============================================================================
...
// ============================================================================
// 3. RENDERIZAR CARDS (DESIGN NOVO COM DATA LADO A LADO)
// ============================================================================
function renderizarCards() {
    ...
    ESCALA_ASSISTENCIA.forEach(item => {
        if (item.separador) {
            divPrazos.innerHTML += `<div class="separador-escala" style="width: 100%; border-bottom: 1px solid rgba(255,255,255,0.1); margin: 15px 0;"></div>`;
            return; 
        }
        ...
        divPrazos.innerHTML += `
            <div class="card-tarefa">
                <div class="tarefa-titulo">${item.funcao}</div>
                <div class="bloco-membro-data" style="display: flex; justify-content: space-between; align-items: center; margin-top: 10px;">
                    <div class="tarefa-responsavel" style="display: flex; align-items: center; gap: 8px; font-size: 0.9rem; color: #ccc;">
                        ${avatarHTML} 
                        <span>${responsavelAtual}</span>
                    </div>
                    <div class="data-card-direita">
                        ${item.prazo}
                    </div>
                </div>
            </div>
