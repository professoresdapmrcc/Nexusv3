...
// ==========================================
// 1. INICIALIZAÇÃO - ACESSO 24/7 LIBERADO
// ==========================================
...
// ==========================================
// 2. NAVEGAÇÃO ENTRE ABAS CORRIGIDA
// ==========================================
...
async function carregarResultados() {
    ...
    try {
        ...
        renderizarGradePropostas(propostas, window.allVotesRaw, lideresNicks, 'resultados-grid');
...
function renderizarGradePropostas(listaProps, todosVotos, lideresNicks, gridId) {
    ...
    if (listaProps.length === 0) html = '<div class="col-span-full text-slate-500 font-bold text-center py-10">Nenhuma proposta em andamento.</div>';
    grid.innerHTML = html;
}
...
document.getElementById('hist-backup-select')?.addEventListener('change', () => {
    ...
    // Renderiza a grade de backup, lideresNicks = new Set() pois é histórico fixo
    renderizarGradePropostas(data.propostas, data.votos, new Set(), 'historico-grid');
});
