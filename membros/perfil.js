document.addEventListener('DOMContentLoaded', () => {
    console.log("✅ perfil.js carregado: Layout melhorado + Modal Full de Fotos + Relacionamentos Habbo.");
    ...
    function openPhotoViewModal(photoId, photoData, isLiked, likeCount) {
        ...
        if (modalDesc) modalDesc.innerHTML  = parseMentions(photoData.description) || "";
        
        const modalTags = document.getElementById('viewModalTags');
        if (modalTags) modalTags.innerHTML  = generateTagsHTML(photoData.taggedUsers);

        // Ajuste para o Modal Novo e Simples de Visualização Rápida
        const modalViewImg = document.getElementById('modal-view-img');
        if (modalViewImg) modalViewImg.src = photoData.imageUrl;
        
        const modalViewDesc = document.getElementById('modal-view-desc');
        if (modalViewDesc) modalViewDesc.innerHTML = parseMentions(photoData.description) || "";

        const modalViewLink = document.getElementById('modal-view-link');
        if (modalViewLink) modalViewLink.href = `/mural.html?photo=${photoId}`;

        // Se o modal de visualização rápida existir, abrir ele
        const viewModalFast = document.getElementById('modal-view-photo');
        if (viewModalFast) {
            viewModalFast.style.display = 'flex';
            document.body.style.overflow = 'hidden';
            return; // Interrompe aqui, não carrega comentários na view rápida
        }

        // --- Código do modal completo original mantido abaixo caso seja utilizado ---
    ...
    // Função global para fechar o Modal Rápido de Fotos
    window.fecharModalFoto = function() {
        const viewModalFast = document.getElementById('modal-view-photo');
        if (viewModalFast) {
            viewModalFast.style.display = 'none';
    ...
    window.closePhotoViewModal = function() {
        ...
        fecharModalFoto(); // Garante o fechamento do modal rápido também
    };
    ...
    const photoViewModalFast = document.getElementById('modal-view-photo');
    if (photoViewModalFast) {
        photoViewModalFast.addEventListener('click', (e) => {
            if (e.target.id === 'modal-view-photo') fecharModalFoto();
        });
    }
    ...
    function renderizarPerfil(data) {
        ...
        const avatarEl = document.getElementById('profile-avatar-img') || document.querySelector('.avatar-card img');
        ...
        setText('profile-role-highlight', data.cargo || 'Membro');
    ...
    // ==========================================
    // 9. RELACIONAMENTOS (Cabecinhas e Tooltip)
    // ==========================================
    function gerarHTMLRelacionamentos(listaRels) {
        ...
        const colleagues = listaRels.filter(r => r.tipo === 'colleague');
        ...
        const criarLinha = (tipo, arrayPessoas) => {
            ...
            const urlHabboHead = `https://www.habbo.com.br/habbo-imaging/avatarimage?user=${principal}&direction=2&head_direction=2&gesture=sml&size=m&headonly=1`;
            const outrosCount  = arrayPessoas.length - 1;
            const textoOutros  = outrosCount > 0 ? `<span class="others">e ${outrosCount} outros</span>` : '';

            // Tooltip para mostrar todos
            let tooltipHtml = '';
            if (arrayPessoas.length > 0) {
                const listItems = arrayPessoas.map(p => {
                    const head = `https://www.habbo.com.br/habbo-imaging/avatarimage?user=${p.nick}&direction=2&head_direction=2&gesture=sml&size=m&headonly=1`;
                    return `<li class="tooltip-item"><div class="tooltip-head"><img src="${head}" alt="${p.nick}"></div><span class="tooltip-name">${p.nick}</span></li>`;
                }).join('');
                tooltipHtml = `<div class="nexus-tooltip"><ul class="tooltip-list">${listItems}</ul></div>`;
            }

            return `<div class="rel-row">
                        <div class="rel-avatar ${classeBadge}">
                            <img src="${urlHabboHead}" alt="${principal}">
                        </div>
                        <div class="rel-text">
                            <a href="/membros/${principal}">${principal}</a> ${textoOutros}
                        </div>
                        ${tooltipHtml}
                    </div>`;
        };

        container.innerHTML = criarLinha('love', loves) + 
                              criarLinha('like', likes) + 
                              criarLinha('colleague', colleagues) + 
                              criarLinha('hate', hates);
    }
    ...
    window.fecharModal = function() {
        ...
        ['modal-frase', 'modal-relacionamentos', 'modal-configuracoes', 'modal-admin-tools'].forEach(id => {
            const el = document.getElementById(id);
    ...
    window.abrirModalFrase = function() {
        ...
        const relModal = document.getElementById('modal-relacionamentos');
        if (relModal) relModal.style.display = 'none';
        
        const palModal = document.getElementById('modal-palette');
        if (palModal) palModal.style.display = 'none';
    };
    ...
    window.abrirModalRelacionamentos = async function(uid) {
        ...
        const fraseModal = document.getElementById('modal-frase');
        if (fraseModal) fraseModal.style.display = 'none';
    ...
    async function salvarFraseNoFirebase(uid) {
        ...
        finally { btnSalvar.textContent = "Salvar"; btnSalvar.disabled = false; }
    }
    ...
    async function carregarListaExclusao(uid) {
        ...
        rels.forEach((rel, index) => {
            ...
            const headUrl = `https://www.habbo.com.br/habbo-imaging/avatarimage?user=${rel.nick}&direction=2&head_direction=2&gesture=sml&size=m&headonly=1`;

            li.innerHTML = `
                <div class="rel-manager-info">
                    <div class="rel-manager-avatar" style="width:35px;height:35px;border-radius:50%;overflow:hidden;background:rgba(0,0,0,0.3);display:flex;justify-content:center;align-items:flex-start;">
                        <img src="${headUrl}" style="margin-top:-5px;">
                    </div>
                    <div class="rel-name-box" style="display:flex;flex-direction:column;">
    ...
    function atualizarVisibilidadeAdministrativa() {
        ...
        firebase.auth().onAuthStateChanged((user) => {
            ...
            window.db.collection('users').doc(user.uid).onSnapshot((snapshot) => {
                adminButton.style.display = snapshot.exists && snapshot.data().role === 'admin'
                    ? 'flex'
                    : 'none';
            }, () => {
                adminButton.style.display = 'none';
            ...
            if (user) {
                try {
                    ...
                    if (userDoc.exists) {
                        ...
                        if (profName) profName.textContent = "Perfil não encontrado.";
                    }
